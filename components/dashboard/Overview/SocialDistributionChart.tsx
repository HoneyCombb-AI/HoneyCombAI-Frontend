...
export function SocialDistributionChart({ title, data, type = "list", colors = DEFAULT_COLORS }: SocialDistributionChartProps) {
    ...
    const getIconForTrend = (name: string) => {
        ...
        if (lower === 'increasing') return <TrendingUp className="h-4 w-4 text-emerald-500" />;
        if (lower === 'decreasing') return <TrendingDown className="h-4 w-4 text-red-500" />;
            ...
            {data.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-2 rounded-lg bg-background/50 border border-border/50">
                    <div className="flex items-center gap-2">
                        {title === "Trend Direction" && getIconForTrend(item.name)}
                        <span className="text-sm font-medium">{item.name}</span>
                    </div>
                    <div className={`px-2 py-0.5 rounded-full text-xs font-medium ${title !== "Trend Direction" ? getBadgeColor(item.name) : 'bg-muted text-muted-foreground'}`}>
                        {item.value}
                    </div>
                </div>
            ))}
