...
export function TestimonialSection(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="relative w-[700px] h-[320px]"
        ...
        >
          {cards.map((card, index) => (
            ...
            >
              <Card
                className="w-full h-full bg-white border-2 border-amber-900/60 shadow-2xl"
                style={{
                  filter: `drop-shadow(0 ${4 + cardTilts[index].depth / 2}px ${
                    8 + cardTilts[index].depth
                  }px rgba(255,195,0,0.3)) drop-shadow(0 ${
                    2 + cardTilts[index].depth / 4
                  }px ${4 + cardTilts[index].depth / 2}px rgba(0,0,0,0.1))`,
                }}
              >
