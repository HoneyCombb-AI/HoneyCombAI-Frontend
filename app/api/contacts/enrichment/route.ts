  ...
  try {
    ...
    try {
      ...
      if (backendResult.status === 'success') {
        ...
        const customMessage = `Contact enrichment initiated for ${body.entity_ids.length} contact${body.entity_ids.length > 1 ? 's' : ''}`
        return NextResponse.json({
          success: true,
          message: customMessage || 'Enrichment request submitted successfully',
